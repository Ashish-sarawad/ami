...
locals {
  ...
  eks_cluster_subnet_ids      = !local.default_network ? local.backend_subnet_ids : local.eks_default_subnet_ids
...
############# Fargate Resource #######################
resource "aws_eks_fargate_profile" "gitlab_fargate_profile" {
  #create                 = var.create_fargate_profile
  cluster_name           = aws_eks_cluster.gitlab_cluster[0].name
  fargate_profile_name   = "${var.prefix}-fargate-Profile"
  pod_execution_role_arn = aws_iam_role.fargate_profile_role.arn
  subnet_ids             = !local.default_network ? local.backend_subnet_ids : local.eks_default_subnet_ids #aws_subnet.example[*].id
  ...
  selector {
   ...
   tags = merge({
    gitlab_node_prefix = var.prefix
    gitlab_profile_type   = "gitlab-fargate-profile"
  }, var.additional_tags)
}
