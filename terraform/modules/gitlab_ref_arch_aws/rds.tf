...
resource "aws_db_instance" "gitlab" {
  ...
  #to use the default parameter group
  # option_group_name       = "default:postgres${local.rds_postgres_major_version}"  # Use the specific parameter group
  ...
  performance_insights_enabled = local.rds_postgres_create ? var.performance_insights_enabled : false
